...
def generate_input_example_new_format(n_workstations=2, n_orders=4, min_products=2, max_products=3, output_dir=None):
    # 1. 生成workStation
    ...
    # 2. 生成产品类型
    n_product_types = random.randint(3, 4)
    product_type_ids = [i+3 for i in range(n_product_types)]  # 如3,4,5
    # 3. 生成每种产品的工序
    operation = {}
    op_id_counter = 59
    for pid in product_type_ids:
        ...
        op_list = []
        for op_seq in range(n_ops):
            ws = random.choice(workstations)
            op_list.append({
                "id": op_id_counter,
                "operationSequence": op_seq + 1,
                "standardTime": round(random.uniform(1.0, 10.0), 2),
                "workstationIdList": [ws["id"]]
            })
            op_id_counter += 1
        operation[str(pid)] = op_list
    ...
    for order_idx in range(n_orders):
        ...
        for _ in range(n_products):
            pid = random.choice(product_type_ids)
