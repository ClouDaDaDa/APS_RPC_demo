...
def spt_eet_rule(env, verbose=False):
    """
    SPT-EET rule: Select the shortest processing time job (SPT) and assign to the earliest end time machine (EET).
    Job selection: Choose job with shortest current operation processing time
    ...
    total_reward = 0
    while not done:
        # Job selection phase - SPT (Shortest Processing Time)
        ...
        if len(available_jobs) == 0:
            ...
            if len(valid_jobs) == 0:
                ...
                # Find all jobs with the minimum processing time
                min_time_jobs = valid_jobs[valid_processing_times == min_processing_time]
                # Randomly select from jobs with minimum processing time
...
def spt_eet_rule_weighted(env, alpha=0.7, verbose=False):
    """
    Priority-weighted SPT-EET rule: Job selection considers priority and processing time, machine assignment uses EET.
    ...
    total_reward = 0
    while not done:
        # Job selection phase - Weighted SPT with priority
        job_mask = obs['agent0']['action_mask']
        job_features = obs['agent0']['observation']
        available_jobs = np.where(job_mask == 1)[0]
        if len(available_jobs) == 0:
            ...
            if np.any(valid_mask):
                ...
                # Normalize (higher priority and shorter processing time are better)
                norm_priority = valid_priorities / (valid_priorities.max() if valid_priorities.max() > 0 else 1)
                norm_processing = 1 - (valid_processing_times - valid_processing_times.min()) / (np.ptp(valid_processing_times) + 1e-6)
...
if __name__ == '__main__':
    ...
    env1.build_and_save_output_json = lambda output_path=None: FjspMaEnv.build_and_save_output_json(env1, output_path or 'Data/OutputData/output_SPT_EET_' + input_case_name)
    makespan1, total_reward1 = spt_eet_rule(env1, verbose=True)
    print(f"[SPT-EET Rule] Makespan: {makespan1}, Total Reward: {total_reward1}")
    print("SPT-EET scheduling output: Data/OutputData/output_SPT_EET_" + input_case_name)

    # Priority-weighted scheduling
    env2 = FjspMaEnv({'inputdata_json': input_data_json})
    env2.build_and_save_output_json = lambda output_path=None: FjspMaEnv.build_and_save_output_json(env2, output_path or 'Data/OutputData/output_SPT_EET_weighted_' + input_case_name)
    makespan2, total_reward2 = spt_eet_rule_weighted(env2, alpha=0.7, verbose=True)
    print(f"[SPT-EET Weighted Rule] Makespan: {makespan2}, Total Reward: {total_reward2}")
    print("SPT-EET weighted scheduling output: Data/OutputData/output_SPT_EET_weighted_" + input_case_name)